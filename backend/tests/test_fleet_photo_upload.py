"""Fleet vehicle photo upload — admin multipart → public URL."""
...
class FleetPhotoUploadTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        import tempfile
        ...
        photo_dir = cls.data_dir / "uploads" / "fleet_photos"
        ...
        cls._orig_photo_dir = admin_mod._FLEET_PHOTO_DIR
        admin_mod._FLEET_PHOTO_DIR = photo_dir
    ...
    def tearDownClass(cls):
        cls.admin_mod._FLEET_PHOTO_DIR = cls._orig_photo_dir
        cls._tmpdir.cleanup()
    ...
    def test_upload_and_public_fetch(self):
        ...
        with patch.dict("os.environ", {"POREIAGO_DATA_DIR": str(self.data_dir)}):
            res = self.client.post(
                "/api/admin/platform/fleet/vehicles/photo-upload",
                files={"file": ("bus.png", io.BytesIO(png), "image/png")},
            )
            self.assertEqual(res.status_code, 200, res.text)
            body = res.json()
            self.assertTrue(body["ok"])
            self.assertTrue(body["url"].startswith("/api/site/fleet-photos/"))
