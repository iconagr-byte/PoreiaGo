...
export default function DriverFormPage() {
  ...
  const header = (
    <div className="flex items-center gap-4 w-full">
      ...
      <h1 className="font-headline-md font-bold flex items-center gap-2">
        ...
        {isEdit ? 'Επεξεργασία οδηγού' : 'Νέος λογαριασμός οδηγού'}
      </h1>
    </div>
  );
  ...
  return (
    <AdminLayout activeTab="drivers" title={header}>
      <form onSubmit={onSubmit} className="max-w-3xl mx-auto pb-20 space-y-6">
        <section className="bg-white rounded-[28px] border border-black/[0.06] shadow-sm p-6 md:p-8 space-y-4">
          <div>
            <h2 className="font-bold text-lg text-gray-900">Στοιχεία οδηγού</h2>
            <p className="text-sm text-gray-500 mt-0.5">
              Βασικά στοιχεία προσωπικού και οχήματος.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label className="block text-sm">
              <span className="font-bold text-gray-700">Email (όνομα χρήστη) *</span>
              <input
                required
                type="email"
        ...
        <section className="bg-sky-50/80 rounded-[28px] border border-sky-100 shadow-sm p-6 md:p-8 space-y-4">
          <div className="flex items-start gap-3">
            <div className="w-11 h-11 rounded-xl bg-sky-100 text-sky-700 flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined">smartphone</span>
            </div>
            <div>
              <h2 className="font-bold text-lg text-sky-950">Λογαριασμός εφαρμογής λεωφορείου</h2>
              <p className="text-sm text-sky-900/80 mt-0.5 leading-relaxed">
                Με αυτά τα στοιχεία μπαίνει ο οδηγός στην εφαρμογή στο{' '}
              ...
              {isEdit && (
                <p className="text-xs font-bold mt-2 text-sky-800">
                  {hasPassword ? '✓ Έχει ήδη κωδικό εφαρμογής' : '⚠ Χωρίς κωδικό — ορίστε έναν παρακάτω'}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label className="block text-sm sm:col-span-2">
              <span className="font-bold text-gray-800">Φωτογραφία οδηγού (URL)</span>
              <input
                type="url"
        ...
        <div className="flex flex-wrap gap-3 sticky bottom-4 z-10">
          <button
            type="submit"
            disabled={saving}
            className="px-6 py-3 rounded-full bg-primary text-white font-bold shadow-lg shadow-primary/25 hover:bg-primary/90 disabled:opacity-60"
          >
            {saving ? 'Αποθήκευση…' : isEdit ? 'Αποθήκευση αλλαγών' : 'Δημιουργία λογαριασμού'}
          </button>
          <Link
            to={isEdit ? `/admin/drivers/${driverId}` : '/admin'}
            state={isEdit ? undefined : { activeTab: 'drivers' }}
            className="px-6 py-3 rounded-full border border-gray-200 bg-white font-bold text-gray-700 hover:bg-gray-50"
          >
            Άκυρο
          </Link>
        </div>
